...
function updateStatusUI(j) {
    ...
    // ── Battery indicator ────────────────────────────────────
    if (j.battery) {
        const bp = j.battery.present;
        const pct = j.battery.percentage;
        const volts = j.battery.voltage;
        ...
        if (bp && typeof pct === 'number') {
            batteryPct.textContent = pct.toFixed(0) + '%';
            batteryVolts.textContent = '(' + volts.toFixed(2) + ' V)';
            batteryFill.style.width = Math.max(0, Math.min(100, pct)).toFixed(0) + '%';
            // Colour coding
            if (pct > 50) {
                batteryFill.style.background = '#4caf50';
                batteryBar.style.background = '#f0f0f0';
            } else if (pct > 20) {
                batteryFill.style.background = '#ff9800';
                batteryBar.style.background = '#fff8e1';
            } else {
                batteryFill.style.background = '#f44336';
                batteryBar.style.background = '#fde0dc';
            }
            batteryIcon.textContent = pct > 50 ? '\u{1F50B}' : pct > 20 ? '\u{1FAAB}' : '\u{1FAAB}';
        } else {
            batteryPct.textContent = '\u2014%';
            batteryVolts.textContent = '(\u2014 V)';
...
frontToggleBtn.addEventListener('click', () => {
    if (frontEnabled) {
        frontEnabled = false;
        frontVideo.src = '';
        frontToggleBtn.textContent = 'Enable Front';
        frontToggleBtn.classList.remove('danger');
        frontToggleBtn.classList.add('primary');
        frontCamStatus.textContent = 'idle';
        ...
        frontCamStatus.textContent = 'connecting...';
    }
});
...
frontVideo.addEventListener('error', () => {
    ...
    if ((now - frontReconnectLastMs) > 2000) {
        ...
        const base = (frontVideo.src || frontVideo.getAttribute('data-src') || '/stream_front.mjpg').split('?')[0];
        frontVideo.src = '';
        setTimeout(() => { if (frontEnabled) frontVideo.src = `${base}?t=${Date.now()}`; }, 100);
    }
}); setInterval(() => {
    if (!frontEnabled) return;
    const now = Date.now();
...
depthToggleBtn.addEventListener('click', async () => {
    if (depthEnabled) {
        ...
        depthEnabled = true;
        depthVideo.src = depthVideo.getAttribute('data-src') || '/stream_depth.mjpg';
